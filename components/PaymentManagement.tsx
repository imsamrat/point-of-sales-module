...
export function PaymentManagement({
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    if (amount > purchase.pendingAmount) {
      toast({
        title: "Validation Error",
        description: `Payment amount cannot exceed pending balance of ৳${purchase.pendingAmount.toFixed(
          2
        )}`,
        variant: "destructive",
      });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-1">
                Purchase from {purchase.supplier.name} - Invoice:{" "}
          ...
          {/* Add Payment Form */}
          {showAddPayment && purchase.pendingAmount > 0 && (
            <Card className="mb-6">
              <CardHeader>
                <CardTitle className="text-lg">Add New Payment</CardTitle>
                <CardDescription>
                  Record a payment for this purchase. Maximum amount:{" "}
                  {formatCurrency(purchase.pendingAmount)}
                </CardDescription>
              </CardHeader>
              <CardContent>
